...
export function HighlightedResumeView({ resumeData, keywords }: HighlightedResumeViewProps) {
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Header */}
      <div className="flex items-center gap-2 p-4 border-b border-border bg-accent">
...
function Section({
  ...
  return (
    <div className="border border-border bg-card rounded">
      <div className="flex items-center gap-2 px-3 py-2 border-b border-border bg-accent">
        {icon}
        <span className="font-mono text-xs font-bold uppercase text-muted-foreground">{title}</span>
      </div>
  ...
  return (
    <span>
      {segments.map((segment, i) =>
        segment.isMatch ? (
          <mark key={i} className="bg-yellow-200 text-foreground px-0.5 rounded-sm">
            {segment.text}
          </mark>
        ) : (
  ...
  return (
    <span
      className={`inline-block px-2 py-0.5 text-xs rounded ${
        isMatch ? 'bg-yellow-200 text-foreground font-medium' : 'bg-accent text-foreground'
      }`}
    >
      {text}
    </span>
