...
function EnhancementCard({ enhancement }: EnhancementCardProps) {
  ...
  return (
    <div className="border-2 border-foreground bg-card shadow-sw-sm">
      {/* Card header */}
      <div className="flex items-center gap-2 px-4 py-3 border-b border-foreground bg-accent">
        {enhancement.item_type === 'experience' ? (
          ...
          <div>
            ...
            <ul className="space-y-2">
              {enhancement.enhanced_description.map((bullet, i) => (
                <li
                  key={i}
                  className="text-sm text-foreground pl-4 border-l-2 border-green-500 bg-green-50 dark:bg-green-950 py-1 pr-2"
                >
                  {bullet}
                </li>
              ))}
