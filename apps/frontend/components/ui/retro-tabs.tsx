...
export const RetroTabs: React.FC<RetroTabsProps> = ({
  ...
  return (
    <div className={cn('flex gap-0 border-b border-foreground', className)}>
      {tabs.map((tab) => {
        ...
        return (
          <button
            ...
            className={cn(
              ...
              'border border-b-0 border-foreground -mb-px',
              'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-700 focus-visible:ring-offset-2',
              isActive && ['bg-card text-foreground font-bold', 'shadow-sw-tab', 'border-b-card'],
              !isActive &&
                !isDisabled && [
                  'bg-secondary text-muted-foreground hover:bg-accent hover:text-foreground',
                ],
              isDisabled && ['bg-accent text-muted-foreground cursor-not-allowed opacity-50']
            )}
          >
            {tab.label}
          </button>
