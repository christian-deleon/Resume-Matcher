...
export const ResumeCard = ({ type, title, lastEdited, onClick }: ResumeCardProps) => {
  ...
  const baseClasses =
    'aspect-[3/4] w-full border-2 border-foreground transition-all duration-200 hover:-translate-y-1 hover:shadow-sw-default cursor-pointer flex flex-col p-6 bg-card';

  if (type === 'new') {
    return (
      <button onClick={onClick} className={`${baseClasses} items-center justify-center group`}>
        <div className="rounded-full border-2 border-foreground p-4">
          <Plus size={32} />
        </div>
  ...
  return (
    <div onClick={onClick} className={baseClasses}>
      <div className="flex-1 bg-accent border border-border mb-4 overflow-hidden relative">
