...
export const SwissGrid = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-[86rem] max-h-full border border-foreground bg-background shadow-sw-lg flex flex-col overflow-hidden">
        {/* Header Section - stays above hovered cards */}
        <div className="border-b border-foreground p-8 md:p-12 shrink-0 bg-background relative z-30">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden relative z-10">
          <div className="p-[1.5px]">
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 bg-border gap-[1px] border-b border-foreground">
              {children}
            </div>
          </div>
        </div>
        ...
        <div className="p-4 bg-background flex justify-between items-center font-mono text-xs text-primary border-t border-foreground shrink-0 relative z-30">
          ...
          <div className="flex items-center gap-4">
            <Link
              href="/settings"
              className="bg-[#F97316] text-black border border-foreground px-6 py-2 uppercase font-bold tracking-wide shadow-sw-sm hover:translate-y-[1px] hover:translate-x-[1px] hover:shadow-none transition-all min-w-[140px] text-center"
            >
              {t('nav.settings')}
            </Link>
          </div>
