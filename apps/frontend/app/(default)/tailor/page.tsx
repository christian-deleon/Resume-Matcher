...
export default function TailorPage() {
  ...
  return (
    <div
      className="min-h-screen w-full bg-background flex flex-col items-center justify-center p-4 md:p-8 font-sans"
      style={{
        backgroundImage:
    ...
    >
      <div className="w-full max-w-4xl bg-card border border-foreground shadow-sw-card p-8 md:p-12 lg:p-14 relative">
        {/* Back Button */}
        ...
        {!statusLoading && !isLlmConfigured && (
          <div className="mb-6 border-2 border-amber-500 bg-amber-50 dark:bg-amber-950 p-4 shadow-sw-default">
        ...
        <div className="space-y-6">
          ...
          <div className="relative">
            <Textarea
              ...
              className="min-h-[300px] font-mono text-sm bg-accent border-2 border-foreground focus:ring-0 focus:border-primary resize-none p-4 rounded-none shadow-inner"
              value={jobDescription}
              onChange={(e) => setJobDescription(e.target.value)}
              onKeyDown={handleTextareaKeyDown}
              disabled={isLoading}
            />
            <div className="absolute bottom-2 right-2 text-xs font-mono text-muted-foreground pointer-events-none">
              {t('tailor.charactersCount', { count: jobDescription.length })}
            </div>
          </div>

          {error && (
            <div className="p-4 bg-red-50 dark:bg-red-950 border border-red-200 text-red-700 text-sm font-mono flex items-center gap-2">
              <span>!</span> {error}
            </div>
          )}
